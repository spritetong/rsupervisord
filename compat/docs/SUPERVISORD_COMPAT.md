...
| :--- | :--- | :--- | :--- | :--- |
| #1 | immediate | **Service Install/Uninstall/Start/Stop/Restart** (Windows + Systemd) | Windows: SCM service, single-binary self-hosting, `service install`/`service uninstall`, `--username/password` login account, immediate AutoStart, `SC_ACTION_RESTART` crash recovery; Systemd: generate `rsupervisord.service` (Restart=always, LimitNOFILE, User/Group, Environment, KillMode aligned), with `service start/stop/restart/uninstall` support; install path and arguments written back into the unit/registry (ExecStart carries an absolute `-c`); uninstall must stop first, then remove, returning non-zero on failure; entry: the `service ...` subcommand on both `rsupervisord` and `rsupervisorctl` (local operation, no daemon connection) | install→start→restart→stop→uninstall passes on both Windows and Linux; no leftover process tree; uninstalling without stopping first returns non-zero + an explicit message |
